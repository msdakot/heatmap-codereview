...
class HeatmapDecorator {
  ...
  applyHeatmap(editor: vscode.TextEditor, heatmapData: HeatmapData) {
    ...
    heatmapData.files.forEach((f, idx) => {
      ...
      let score = 0;
      if (normalizedEditorPath.endsWith(prPath)) score = 3;
      else if (editorBase === prBase) score = 2;
      else if (normalizedEditorPath.includes(prPath)) score = 1;
      if (score > bestScore) {
        bestScore = score;
        bestIdx = idx;
      }
