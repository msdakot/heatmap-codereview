...
exports.PRReviewerService = void 0;
const github_1 = require("./github");
const coderabbit_cli_1 = require("./coderabbit-cli");
const heatmap_1 = require("./heatmap");
class PRReviewerService {
    constructor(githubToken, openaiApiKey) {
        this.githubService = new github_1.GitHubService(githubToken);
        this.codeRabbitService = new coderabbit_cli_1.CodeRabbitCLIService();
        this.heatmapService = new heatmap_1.HeatmapService(openaiApiKey);
    }
    async generateHeatmapReview(request) {
            ...
            try {
                ...
                if (request.includeCodeRabbit) {
                    try {
                        ...
                        codeRabbitComments = await this.codeRabbitService.reviewFile(file.filename, fileContent, file.patch);
                    }
                    catch (error) {
                        console.warn(`CodeRabbit review failed for ${file.filename}:`, error);
                        // Fallback to mock comments
                        codeRabbitComments = await this.codeRabbitService.mockReviewFile(file.filename, '', file.patch);
                    }
                }
                ...
                Object.keys(fileSeverityDistribution).forEach(severity => {
                    heatmapData.severityDistribution[severity] +=
                        fileSeverityDistribution[severity];
                });
            }
            catch (error) {
...
exports.PRReviewerService = PRReviewerService;
